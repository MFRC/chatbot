...
class CustomerResourceIT {
    ...
    @Test
    @Transactional
    void getAllCustomersByBookingIsEqualToSomething() throws Exception {
        Booking booking;
        ...
        // Get all the customerList where booking equals to bookingId
        defaultCustomerShouldBeFound("bookingId.equals=" + bookingId);

        // Get all the customerList where booking equals to UUID.randomUUID()
        defaultCustomerShouldNotBeFound("bookingId.equals=" + UUID.randomUUID());
    }
    ...
    void getAllCustomersByPaymentIsEqualToSomething() throws Exception {
        ...
        // Get all the customerList where payment equals to UUID.randomUUID()
        defaultCustomerShouldNotBeFound("paymentId.equals=" + UUID.randomUUID());
    }

    @Test
    @Transactional
    void getAllCustomersByRepairRequestIsEqualToSomething() throws Exception {
        RepairRequest repairRequest;
        if (TestUtil.findAll(em, RepairRequest.class).isEmpty()) {
            customerRepository.saveAndFlush(customer);
            repairRequest = RepairRequestResourceIT.createEntity(em);
        } else {
            repairRequest = TestUtil.findAll(em, RepairRequest.class).get(0);
        }
        em.persist(repairRequest);
        em.flush();
        customer.addRepairRequest(repairRequest);
        customerRepository.saveAndFlush(customer);
        UUID repairRequestId = repairRequest.getId();

        // Get all the customerList where repairRequest equals to repairRequestId
        defaultCustomerShouldBeFound("repairRequestId.equals=" + repairRequestId);

        // Get all the customerList where repairRequest equals to UUID.randomUUID()
        defaultCustomerShouldNotBeFound("repairRequestId.equals=" + UUID.randomUUID());
    }
